...
class WishListViewModel: ObservableObject {
    ...
    init(userID: String) {
        fetchWishlistBooks(userID: userID)
    }

    func fetchWishlistBooks(userID: String) {
        db.child("users").child(userID).child("wishlist").observe(.value, with: { snapshot in
                    var newBooks = [Book]()
